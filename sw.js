...
// ==========================================
const CACHE_NAME = 'diccionario-acordes-v1'; 
...
self.addEventListener('fetch', event => {
  event.respondWith(
    caches.match(event.request)
      .then(response => {
        // Si el archivo ya está en el celular, lo devuelve al instante (0 megas gastados)
        if (response) {
          return response;
        }
        // Si no está (ejemplo: un error), intenta ir a buscarlo a internet
